...
We will continue from where we left off in [the part three][third-part] of this tutorial.

It is time to give our stored procedure the freedom of accessing the internet. By default Snowflake does not allow stored procedures to access the internet. We must enable it by creating [external access integration](#external-access-integration) and attaching it to our stored procedure.

## External Access Integration

[External access integration](https://docs.snowflake.com/en/developer-guide/external-network-access/external-network-access-overview) is a Snowflake object. It gives stored procedures and user defined functions **granular access** to the internet. External access integration in itself is a container that contains [network rules](#network-rule) and [secrets](#secret).
...
A [network rule](https://docs.snowflake.com/en/sql-reference/sql/create-network-rule) is a Snowflake object. It represents the external network’s location and restrictions for access. We can specify which domains, ip address or ip address ranges are allowed/disallowed, and whether the communication is ingress (coming from the external source) or egress (going to external source).
...
Instead of hard-coding api keys, we can use Snowflake's [secrets](https://docs.snowflake.com/en/sql-reference/sql/create-secret) feature. It is similar to GitHub Secrets or AWS secrets if you used either of them. Once a secret is created, we can access it from our stored procedure.
...
[session.table(name=table_name)](https://docs.snowflake.com/en/developer-guide/snowpark/reference/python/latest/snowpark/api/snowflake.snowpark.Session.table#snowflake.snowpark.Session.table) returns dataframe that points to underlying Snowflake table.

[snowpark.functions](https://docs.snowflake.com/en/developer-guide/snowpark/reference/python/latest/snowpark/functions) module provides SQL functions like aggregation, sorting and other utilities that generate Column expressions you can pass to DataFrame transformation methods.
...
If you split it into multiple stored procedures, you can call another stored procedure withing a stored procedure using [session.call()](https://docs.snowflake.com/en/developer-guide/snowpark/reference/python/latest/snowpark/api/snowflake.snowpark.Session.call.html#snowflake.snowpark.Session.call) function.

## Scheduling Stored Procedure Execution

Our stored procedure is complete. Now, we can run it at anytime we want. But scheduling its execution is a much better idea. We can do so using [Snowflake Tasks](https://docs.snowflake.com/en/user-guide/tasks-intro).
